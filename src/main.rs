...
fn main() {
    ...
    init_logging();
    info!(
        "Hi, I am {} at your service, this is version {}",
    ...
    info!(
        "We're done, goodbye! This took {} seconds ",
        duration.to_string()
    );
}
...
fn process_noted_csv(noted_folder: &str, done_folder: &str, xero_folder: &str) {
    info!(
    ...
    for path in paths {
        if let Ok(entry) = path {
            ...
            match copy_result {
                Ok(_) => {
                    info!(
                        "Processing complete, copying file {} to location {}",
                        entry.display().to_string(),
            ...
            panic!("Could not get a valid path to read from, leaving now");
        }
    }
    if path_counter == 0 {
...
fn write_xero_csv(xero_lines: Vec<XeroType>, target_path: &str) {
    ...
    match header_result {
        Ok(_) => {
            info!("Written Xero headers")
        }
        Err(err) => {
            error!("Could not write headers, skipping this, reason {:?}", err)
        }
    }
    for xero_item in xero_lines.iter() {
        ...
        match write_record_result {
            ...
            Err(err) => {
                error!("This record has been skipped: {:?}", err)
            }
        }
    }
    ...
    match flush_result {
        Ok(_) => {
            info!("Stored Xero csv at {}", filepath)
        }
        Err(err) => {
            error!("Could not save xero file {} because: {:?}", &filepath, err)
        }
    }
}
